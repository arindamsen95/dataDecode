...
void
vetrocDataDecode(uint32_t data)
{
  ...
  switch( type_current )
	...
	if( new_type )
	  {
	    vetroc_trigger_time_1_t d; d.raw = data;
	  ...
	  {
	    vetroc_trigger_time_2_t d; d.raw = data;
	    if( time_last == 1 )
	      {
    ...
    case 8:		/* PULSE TIME */
      {
	vetroc_pulse_time_t d; d.raw = data;

	  printf("%8X - PULSE TIME - edge = %d   chan = %d   time = %d\n",
		 d.raw,
		 d.bf.edge,
		 d.bf.channel,
		 d.bf.time);
